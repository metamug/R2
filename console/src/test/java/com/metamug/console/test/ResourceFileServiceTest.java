 ...
 * @author Kainix
 */
public class ResourceFileServiceTest {

    ResourceFileService fileService = new ResourceFileService();

    public ResourceFileServiceTest() {
    }

    //    @Test
    public void testGetFolderListing() {
        try {
            System.out.println("getFolderListing");
            int userId = 1;
            String appName = "test";
            JSONObject result = fileService.listing(userId, appName);
            System.out.println(result);
        } catch (IOException | SQLException | PropertyVetoException | ClassNotFoundException ex) {
            Logger.getLogger(ResourceFileServiceTest.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
...
//    @Test
    public void testReadableFileSize() {
        ...
        long size = 0L;
        String result = ResourceFileService.getFileSize(size);
    }
...
//    @Test
    public void testAddFile() throws Exception {
        ...
        props.put("version", "v1.0");
        props.put("secure", false);
        boolean result = fileService.addFile(userId, appName, fileName,
                fileSize, props, null);
    }
...
//    @Test
    public void testDownloadFile() throws Exception {
        ...
        String version = "v1.0";
        String fileName = "";
        JSONObject result = fileService.downloadFile(userId, appName, fileName, version);
    }
...
//    @Test
    public void testDeleteFile() throws Exception {
        ...
        domain.append(Util.REQUEST_SCHEME).append("://").append(Util.LOCALHOST_IP);
        domain.append(":").append(Util.SERVER_PORT);
        fileService.deleteFile(version, fileName, appName, userId, domain.toString(), null);
    }
}
