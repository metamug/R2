...
public class ConnectionProvider {
    ...
    private ConnectionProvider(Properties dbProps) throws ClassNotFoundException, SQLException {
        String JDBC_DRIVER = dbProps.getProperty("driver") == null ? "" : dbProps.getProperty("driver").trim();
        String protocol = dbProps.getProperty("protocol") == null ? "" : dbProps.getProperty("protocol").trim();
        String db = dbProps.getProperty("dbname") == null ? "" : dbProps.getProperty("dbname").trim();
        String userName = dbProps.getProperty("username") == null ? "" : dbProps.getProperty("username").trim();
        String host = dbProps.getProperty("host") == null ? "" : dbProps.getProperty("host").trim();
        String password = dbProps.getProperty("password") == null ? "" : dbProps.getProperty("password").trim();
        String dbUrl;
        if (JDBC_DRIVER.contains("hsqldb")) {
            if (host.isEmpty()) {
                host = Util.APP_BASE + "/databases/mtg_console";
            }

            dbUrl = protocol + "://" + host + "/" + db + (dbProps.getProperty("options") != null && !dbProps.getProperty("options").trim().isEmpty() ? ";" + dbProps.getProperty("options").trim() : "");
        } else {
            dbUrl = protocol + "://" + host + "/" + db + (dbProps.getProperty("options") != null && !dbProps.getProperty("options").trim().isEmpty() ? "?" + dbProps.getProperty("options").trim() : "");
        }
    ...
    public static ConnectionProvider getInstance() throws IOException, SQLException, PropertyVetoException, ClassNotFoundException {
        Properties dbProps = Util.getDbProperties();
        return new ConnectionProvider(dbProps);
    }
    ...
    public Connection getConnection(String dbtype, String dburl, String user, String pass) throws ClassNotFoundException, SQLException {
        switch (dbtype) {
            case Util.HSQLDB:
                Class.forName(Util.HSQLDB_JDBC_DRIVER);
                if (!dburl.contains("jdbc:")) {
                    dburl = "jdbc:hsqldb:file://" + dburl;
                }
                return DriverManager.getConnection(dburl, user, pass);
            case Util.MYSQL:
                Class.forName(Util.MYSQL_JDBC_DRIVER);
                if (!dburl.contains("jdbc:")) {
                    dburl = "jdbc:mysql://" + dburl;
                }
                return DriverManager.getConnection(dburl, user, pass);
            case Util.POSTGRESQL:
                Class.forName(Util.POSTGRESQL_JDBC_DRIVER);
                if (!dburl.contains("jdbc:")) {
                    dburl = "jdbc:postgresql://" + dburl;
                }
                return DriverManager.getConnection(dburl, user, pass);
            case Util.MSSQL:
                Class.forName(Util.MSSQL_JDBC_DRIVER);
                if(!dburl.contains("jdbc:")){
