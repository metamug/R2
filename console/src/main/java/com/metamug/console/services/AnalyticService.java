...
public class AnalyticService {
    ...
    public JSONObject getStats(int userId, String appName, String domain) throws IOException {
        String result = Util.executeQueryInApp(domain + "/" + appName, "stats", "", appName, null);
        ...
        if (result.length() > 0) {
            try {
                ...
                emptyJson.put("resourcecount", new JSONArray());
                emptyJson.put("dailycount", new JSONArray());
                return emptyJson;
            }
        } else {
            JSONObject emptyJson = new JSONObject();
...
//        return dao.getStats(userId, appName);
    }
}
