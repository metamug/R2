 ...
 */
package com.metamug.console.services.file;

import com.metamug.console.daos.file.ResourceFileDAO;
...
public class ResourceFileService {
    ...
    private final ResourceFileDAO dao;

    public ResourceFileService() {
        this.dao = new ResourceFileDAO();
    }

    public boolean addFile(int userId, String appName, String fileName, String fileSize,
            JSONObject props, Boolean deploy)
                    throws IOException, SQLException, PropertyVetoException, ClassNotFoundException {
        return dao.addFile(userId, appName, fileName, fileSize, props, deploy);
    }
    ...
    public JSONObject downloadFile(int userId, String appName, String fileName, String version) throws IOException, SQLException, PropertyVetoException, ClassNotFoundException {
        return dao.downloadFile(userId, appName, fileName, version);
    }
    ...
    public JSONObject listing(int userId, String appName) throws IOException, SQLException, PropertyVetoException, ClassNotFoundException {
        return dao.listing(userId, appName);
    }
    ...
    public JSONObject deleteFile(String version, String fileName, String appName, int userId, String domain, String type) throws IOException, SQLException, PropertyVetoException, ClassNotFoundException {
        return dao.deleteFile(version, fileName, appName, userId, domain, type);
    }
    ...
    public static String getFileSize(float bytes) {
        ...
        float petabytes = (terabytes / 1024);
        float exabytes = (petabytes / 1024);
        float zettabytes = (exabytes / 1024);
        float yottabytes = (zettabytes / 1024);

        if (yottabytes > 1) {
            return buff.append(format.format(yottabytes)).append("YB").toString();
        } else if (zettabytes > 1) {
            return buff.append(format.format(zettabytes)).append("ZB").toString();
        } else if (exabytes > 1) {
            return buff.append(format.format(exabytes)).append("EB").toString();
        } else if (petabytes > 1) {
            return buff.append(format.format(petabytes)).append("PB").toString();
        } else if (terabytes > 1) {
            return buff.append(format.format(terabytes)).append("TB").toString();
        } else if (gigabytes > 1) {
            return buff.append(format.format(gigabytes)).append("GB").toString();
        } else if (megabytes > 1) {
            return buff.append(format.format(megabytes)).append("MB").toString();
        } else if (kilobytes > 1) {
            return buff.append(format.format(kilobytes)).append("kB").toString();
        } else {
            return buff.append(format.format(kilobytes)).append("B").toString();
        }
    }
    
    public static File getResourceXmlFile(String appName, String fileName, String version){
        String filepath = Util.XML_RESOURCE_FOLDER + File.separator + appName + File.separator + version 
