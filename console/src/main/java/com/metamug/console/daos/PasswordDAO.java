...
public class PasswordDAO {
    ...
    public int resetPassword(String password, String emailId) {
        int resultStatus = 0;
        try (Connection con = ConnectionProvider.getInstance().getConnection()) {
            PreparedStatement statement = con.prepareStatement("UPDATE console_user SET password=? WHERE email_id=? AND deleted_on IS NULL");
    ...
    public int changePassword(int userId, String oldPassword, String newHashPassword) throws IOException, SQLException, PropertyVetoException, ClassNotFoundException {
        int resultStatus = 0;
        try (Connection con = ConnectionProvider.getInstance().getConnection()) {
            try (PreparedStatement statement = con.prepareStatement("SELECT password FROM console_user WHERE user_id=? AND deleted_on is NULL")) {
                statement.setInt(1, userId);
                try (ResultSet result = statement.executeQuery()) {
                    while (result.next()) {
                        String pswd = result.getString("password");
                        boolean checkpw = BCrypt.checkpw(oldPassword, pswd);
                        if (checkpw) {
                            try (PreparedStatement stmt = con.prepareStatement("UPDATE console_user SET password=? WHERE user_id=? AND deleted_on IS NULL")) {
