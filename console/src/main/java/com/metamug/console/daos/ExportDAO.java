...
public class ExportDAO {
    ...
    public JSONObject getAppDetails(String appName) {
        JSONObject details = new JSONObject();
        try (Connection con = ConnectionProvider.getInstance().getConnection()) {
            PreparedStatement statement = con.prepareStatement("SELECT app_version,app_db,created_on FROM console_app WHERE app_name=? AND deleted_on IS NULL");
            statement.setString(1, appName);
            try (ResultSet result = statement.executeQuery()) {
                while (result.next()) {
                    details.put("app_name", appName);
                    details.put("app_version", result.getString("app_version"));
                    details.put("created_on", result.getString("created_on"));
                }
            }
