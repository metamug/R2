...
//    @Deprecated
//    public JSONObject getStats(int userId, String appName) {
//        JSONObject obj = new JSONObject();
//        try (Connection con = ConnectionProvider.getInstance().getConnection()) {
//            PreparedStatement statement = con.prepareStatement("SELECT app_name FROM console_app WHERE app_name=? AND user_id=?");
...
//            if (result.next()) {
//                PreparedStatement stmt = con.prepareStatement("SELECT app_name,resource,version,status,count(log_id) as count FROM console_log GROUP BY app_name,resource,version HAVING status <> 404 AND app_name=?");
...
//                }
//                PreparedStatement stmnt = con.prepareStatement("SELECT count(log_id) AS count, DATE(logged_on) AS date,app_name FROM (SELECT log_id,app_name,logged_on FROM console_log WHERE app_name=? AND logged_on >= now()-interval 1 month) AS log GROUP BY DATE(logged_on),app_name ORDER BY DATE(logged_on) ASC");
...
//            obj.put("status", 409);
//            Logger.getLogger(AnalyticDAO.class.getName()).log(Level.SEVERE, ex.getLocalizedMessage(), ex);
//        }
//        return obj;
//    }
}
