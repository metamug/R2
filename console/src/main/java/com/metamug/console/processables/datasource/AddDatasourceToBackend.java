 ...
 */
package com.metamug.console.processables.datasource;
 ...
 */
public class AddDatasourceToBackend implements RequestProcessable {
    @Override
    public Response process(Request req, DataSource ds, Map<String, Object> arg) throws Exception {
        Map<String,String> params = req.getParams();
        String name = params.get("dsName");
        ...
        String domain = (String)arg.get("domain");
        //add datasource to backend config
        ConfigManager backendConfig = new ConfigManager(appName);
        boolean added = backendConfig.addExternalDatasource(user, password, type, url, name);
        
        JSONObject result = new JSONObject();
        
        if(added){
            //re deploy backend
            Loader backendLoader = new Loader(appName);
            backendLoader.reload(domain);
            
            result.put("success", true);
            result.put("message", "Datasource "+name+" added successfully to "+appName);
        }else{
            result.put("success", false);
            result.put("message", "Could not add datasource. Please check connection URL!");
        }
