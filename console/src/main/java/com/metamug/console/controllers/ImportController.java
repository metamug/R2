...
import com.metamug.console.services.ImportService;
 ...
 */
@WebServlet(name = "ImportController", urlPatterns = {"/import"})
public class ImportController  extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ...
        try {
            ...
            if (isVerified) {
                if (isMultipart) {
                    File importZip = RequestUtil.getUploadedFile(request);
                    if (importZip != null) {
                        ImportService service = new ImportService();
                        service.importApp(userId, importZip, domain);
            ...
            Logger.getLogger(ImportController.class.getName()).log(Level.SEVERE, ex.getMessage(), ex);
        } catch (FileUploadException ex) {
            obj.put("message", "Zip File can't be uploaded. Try again");
            obj.put("status", 502);
            response.setStatus(502);
        } catch (FileNotFoundException ex) {
            if(ex.getMessage().contains("config.json")){
                obj.put("message", "Could not find <span class=\"text-success\">config.json</span> file!");
            }else{
                obj.put("message", ex.getMessage());
            }
            obj.put("status", 422);
            ...
            Logger.getLogger(ImportController.class.getName()).log(Level.SEVERE, msg, ex);
            ...
            String error = "Could not read <span class=\"text-success\">config.json</span> file!";
            if(msg.contains("not found")){
                if(msg.contains("url")){
                    error = "Could not find \"url\" object in <span class=\"text-success\">config.json</span> file!";
                }else if(msg.contains("type")){
                    error = "Could not find \"type\" object in <span class=\"text-success\">config.json</span> file!";
                }else if(msg.contains("user")){
                    error = "Could not find \"user\" object in <span class=\"text-success\">config.json</span> file!";
                }else if(msg.contains("app_db_details")){
                    error = "Could not find \"app_db_details\" object in <span class=\"text-success\">config.json</span> file!";
                }else if(msg.contains("pw")){
                    error = "Could not find \"pw\" object in <span class=\"text-success\">config.json</span> file!";
                }else if(msg.contains("app_version")){
                    error = "Could not find \"app_version\" object in <span class=\"text-success\">config.json</span> file!";
                }
            }
