...
import com.metamug.console.services.AnalyticService;
 ...
 * @author Kaisteel
 */
@WebServlet(name = "AnalyticController", urlPatterns = {"/stat/*"})
public class AnalyticController extends HttpServlet {
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        AnalyticService statService = new AnalyticService();
        try {
            if (pathInfo != null) {
                ...
                switch (path.length) {
                    case 2:
                        ...
                        if (userService.isVerifiedUser(userId)) {
                            ...
                            //url: /{appName}
                            obj = statService.getStats(userId, appName, domain);
                            JSONObject appData = new AppService().getApp(appName, userId, false);
                            obj.put("backendData", appData.getJSONArray("data").getJSONObject(0));
                        } else {
            ...
            Logger.getLogger(AnalyticController.class.getName()).log(Level.SEVERE, ex.getMessage(), ex);
        ...
        try (ServletOutputStream out = response.getOutputStream()) {
            ...
            Logger.getLogger(AnalyticController.class.getName()).log(Level.SEVERE, ex.getLocalizedMessage(), ex);
        }
    }
