...
import static com.metamug.console.util.Util.XML_RESOURCE_FOLDER;
 ...
 */
public class OpenApiJson implements RequestProcessable {

    @Override
    public Response process(Request r, DataSource ds, Map<String, Object> args) {
        String appname = (String)args.get("appname");
        String appdesc = (String)args.get("appdesc");
        Backend backend = new Backend(appname, appdesc, "");
        
        @SuppressWarnings("unchecked")
        SortedMap<String,String>[] resources = (SortedMap<String,String>[])args.get("resources");
       
        for(SortedMap resource : resources){
            try {
                String name = (String)resource.get("FILE_NAME");
                String v = (String)resource.get("FILE_VERSION");
                System.out.println(name);
                System.out.println(v);
                String filepath = XML_RESOURCE_FOLDER+File.separator+appname+File.separator+v+File.separator+
                        name+".xml";
                System.out.println(filepath);
                File file = new File(filepath);
                
                backend.addResource(appname+"/"+v+"/"+name, ResourceParser.generateResource(file));
                
            } catch (IOException | SAXException | JAXBException ex) {
                Logger.getLogger(OpenApiJson.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
