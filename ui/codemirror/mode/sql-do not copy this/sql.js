...
const reservedSqlKeywords = ['SELECT','WHERE','FROM','LIMIT','LEFT','RIGHT','JOIN','OFFSET',
...
CodeMirror.defineMode("sql", function(config, parserConfig) {
  ...
  return {
    ...
    token(stream, state) {
      ...
      for(let i = 0;i<reservedSqlKeywords.length;i++){
        const rsql = reservedSqlKeywords[i];

        if (stream.match(`${rsql} `,true,true) ) {
            return "sqlkeyword"
        }
      }
